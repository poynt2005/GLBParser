...
namespace
{
    ...
    template <typename T>
    void SerializeAccessor(const Accessor &accessor, const Document &doc, const GLTFResourceReader &reader, BufferBuilder &builder)
    {
    ...
    void SerializeAccessor(const Accessor &accessor, const Document &doc, const GLTFResourceReader &reader, BufferBuilder &builder)
    {
        ...
        {
        case COMPONENT_BYTE:
            SerializeAccessor<int8_t>(accessor, doc, reader, builder);
            break;
        case COMPONENT_UNSIGNED_BYTE:
            SerializeAccessor<uint8_t>(accessor, doc, reader, builder);
            break;
        case COMPONENT_SHORT:
            SerializeAccessor<int16_t>(accessor, doc, reader, builder);
            break;
        case COMPONENT_UNSIGNED_SHORT:
            SerializeAccessor<uint16_t>(accessor, doc, reader, builder);
            break;
        case COMPONENT_UNSIGNED_INT:
            SerializeAccessor<uint32_t>(accessor, doc, reader, builder);
            break;
        case COMPONENT_FLOAT:
            SerializeAccessor<float>(accessor, doc, reader, builder);
            break;
        default:
            throw GLTFException("Unsupported accessor ComponentType");
        }
                                               ...
                                               std::shared_ptr<const IStreamWriter> outputStreamWriter)
{
    ...
    std::unique_ptr<BufferBuilder> builder = std::make_unique<BufferBuilder>(
        std::move(writer),
        [](const BufferBuilder &)
        { return GLB_BUFFER_ID; },
        [&currentBufferViewIdStr](const BufferBuilder &)
        { return currentBufferViewIdStr; },
        [&currentAccessorIdStr](const BufferBuilder &)
        { return currentAccessorIdStr; });
    ...
    for (const auto &accessor : document.accessors.Elements())
    {
        ...
        {
            SerializeAccessor(accessor, document, resourceReader, *builder);
            AdvanceBufferViewId();
        }
        else
        {
...
void Microsoft::glTF::Toolkit::SerializeBinary(const Document &document, std::shared_ptr<const IStreamReader> inputStreamReader,
                                               std::shared_ptr<const IStreamWriter> outputStreamWriter)
{
    SerializeBinary(document, GLTFResourceReader{inputStreamReader}, std::move(outputStreamWriter));
}